-- =============================================
-- AgênciaForge — Corrige fuso horário e transição no auto-agendamento
-- Problema: (date + time)::timestamptz usava UTC, mas o usuário salva
-- os horários no fuso de Brasília (America/Sao_Paulo = UTC-3).
-- Resultado: item marcado para 15:00 era agendado às 12:00.
...
BEGIN
  ...
  -- CORREÇÃO: interpreta scheduled_date + scheduled_time como horário de Brasília
  -- (America/Sao_Paulo) e converte para timestamptz (UTC internamente).
  -- Sem isso, o Supabase (UTC) agendava 3 horas antes do horário pretendido.
  v_scheduled_at := (
  ...
  -- ── Marca o item do planner como agendado ───────────────────────────────────
