-- =============================================
-- AgênciaForge — Auto-agendamento Instagram via trigger
-- Agenda automaticamente no servidor quando cliente aprova,
...
BEGIN
  ...
  IF NEW.art_approval_status IS NOT NULL OR NEW.copy_approval_status IS NOT NULL THEN
    IF NEW.art_approval_status = 'aprovado' AND NEW.copy_approval_status = 'aprovado' THEN
  ...
  v_scheduled_at := (
    ...
    + COALESCE(NEW.scheduled_time::time, '12:00'::time)
  )::timestamptz;
...
-- BEFORE UPDATE: a mudança de ig_scheduled=true é aplicada na mesma transação
